...
export function formatResponse(responseType: ResponseType, status: number, message: string, errors?: Record<string, string[]>) {
...
export function handleError(error: unknown, responseType: ResponseType = 'server') {
  ...
  if (error instanceof ZodError) {
    ...
    logger.error({ err: validationError }, `${responseType.toUpperCase()} ValidationError: ${validationError.message}`);
